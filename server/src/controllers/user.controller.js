...
async function add_User(req, res) {
  ...
  try {
    ...
    if (user) {
      ...
      const secret ="importantsecret"+email;
      const token=sign( {email:email},secret,{expiresIn:"1h"});
      const confirmationLink = `http://localhost:5000/users/confirm-user-creation?token=${token}&email=${email}`;
      const emailSent = await verifyAccount(email, confirmationLink);

      if (!emailSent) {
        return res.status(500).json({ error: 'Error sending confirmation email' });
      }

      // Create a new user with status 'en attente de confirmation'
      ...
      res.json({ message: "Please confirm your email address to complete registration" });
    }
  } catch (err) {
    console.error(err);
...
//route pour confirme user creation
async function confirm_User_Creation(req, res) {
  const { token, email: userEmail } = req.query;

  try {
    const secret = "importantsecret" + userEmail;
    ...
    res.json({ message: "Your email has been verified. Your account is now active." });
  } catch (err) {
    console.error(err);
    res.status(400).json({ error: err });
  }
...
async function login_user(req, res) {
    ...
    if (!user) {
      ...
      bcrypt.compare(password, dbpassword).then((match) => {
        if(user.status==="wating for confirmation"){res.json({error:"you should verify your account first"});}
        else if (!match) {
...
var transporter = nodemailer.createTransport({
  service: 'gmail',
  auth: {
    user: 'eveypodcast@gmail.com',
    pass: 'cszirdxnnyuhelec'
  }
});

var mailOptions = {
  from: 'eveypodcast@gmail.com',
  to: email,
...
// Fonction pour vérifier le compte de l'utilisateur
async function verifyAccount(userEmail, confirmationLink) {
  try {
    ...
    const mailOptions = {
      ...
      text: `Click on this link to confirm your account : ${confirmationLink}`,
    };
...
async function sendConfirmationEmail(userEmail, confirmationLink) {
  try {
    ...
    const mailOptions = {
      from: 'eveypodcast@gmail.com',
      to: userEmail,
      subject: 'Confirmation deleting account',
      text: `Click on this link to delete your account : ${confirmationLink}`,
    };
...
module.exports = {
   ...
   update_alerts,getTopActiveUsers,confirm_User_Creation
};
